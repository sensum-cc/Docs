...
async function getDownloadLink(sharedSession, useProxy = false) {
  ...
  if (useProxy) {
    ...
    url = `${downUrl}&skuId=${skuId}&sessionId=${
      sharedSession ? sharedSessionGUID : sessionId.value
    }`;
  }
...
async function getDownload() {
  ...
  async function useProxy() {
    ...
    try {
      console.warn(
        "ISO Downloader: Getting ISO from Microsoft failed, attempting to use the proxy..."
      );
      isoLink = await getDownloadLink(false, true);
    } catch (e) {
      // Everything failed, show error
      console.error(
        "ISO Downloader: Retrieving the ISO from both Microsoft and the proxy failed, showing error to the user..."
      );
  ...
  try {
    console.info(
      "ISO Downloader: Getting download from Microsoft without shared session..."
    );
    ...
    try {
      console.warn(
        "ISO Downloader: Getting ISO from Microsoft without shared session failed, attempting using shared session..."
      );
  ...
  if (!isoLink) {
    ...
    isoLink.startsWith("https://software.download.prss.microsoft.com")
  ) {
...
function getLangError(msFailure) {
  setTimeout(() => {
    ...
    if (getLangFailCount > langAttempt) {
      ...
      console.warn(
        `ISO Downloader: Failed to get langauges ${getLangFailCount} times, and Microsoft's response failing is ${msFailure}. Max tries is ${langAttempt}.`
      );
      getLanguages(true);
    }
  }, 1000);
...
function getLanguages(sharedSession = false) {
  ...
  xhr.open(
    "GET",
    `${langsUrl}&productEditionId=${winProductID}&sessionId=${
      sharedSession ? sharedSessionGUID : sessionId.value
    }`,
    true
  );
